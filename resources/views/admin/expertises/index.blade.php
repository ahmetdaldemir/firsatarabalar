...
@section('content')
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col">
            <h2>Ekpertiz Yönetimi</h2>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="/">Ana Sayfa</a></li>
                <li class="breadcrumb-item active"><strong>Ekpertizler</strong></li>
            </ol>
        </div>
        <div class="col-6 d-flex justify-content-end align-items-center" style="padding-top: 52px">
            ...
            <div class="d-flex justify-content-end align-items-center">
                <div class="custom-control custom-checkbox">
                    ...
                    <label class="custom-control-label" for="show_deleted">Silinen Ekpertizleri Göster</label>
                </div>
                @if (!old('q') )
                    <a href="{{route('admin.expertises.index')}}" class="btn btn-xs btn-warning ml-3"><i
                                class="fad fa-trash-alt mr-1"></i> Arama Filtresini Kaldır</a>
                @endif
                <a href="{{route('admin.expertises.create')}}" class="btn btn-xs btn-success mr-2"><i
                            class="fad fa-plus-circle mr-1"></i> Yeni Ekpertiz Ekle</a>
            </div>
    ...
    <div class="wrapper wrapper-content">
        ...
        <table id="Users" class="table table-striped table-bordered table-hover mb-0 bg-white">
            ...
            <tr>
                ...
                <th width="180" class="text-center">Adres</th>
                <th width="130" class="text-center">Fiyat</th>
            ...
            @else
                @foreach ( $expertises as $expertis)
                    <tr id="customerRow-{{$expertis->id}}">
                        <td class="text-center">{{$expertis->name}}</td>
                        <td class="text-center">{{$expertis->phone}}</td>
                        <td>{{$expertis->email}}</td>
                        <td class="text-center">{{$expertis->il}} / {{$expertis->ilce}}</td>
                        <td class="text-center">{{$expertis->address}}</td>
                        <td class="text-center">{!! ( $expertis->status == 1 ) ? '<a href="'.route('admin.expertises.status',['id'=>$expertis->id,'status'=>0]).'"><span class="label label-primary">Aktif</span></a>' : '<a href="'.route('admin.expertises.status',['id'=>$expertis->id,'status'=>1]).'"><span class="label label-plain">Pasif</span></a>' !!}</td>
                        <td class="text-center">
                            @if ( Request::get("deleted") == "show" )
                                <a href="javascript:;" onclick="undeleteCustomer({{$expertis->id}});"
                            ...
                            @else
                                <a href="javascript:;" data-toggle="modal" data-target="#SendSms"
                                   data-customernumber='{{$expertis->phone}}' class="sendsms btn btn-xs btn-primary"><i
                                            class="far fa-envelope"></i></a>
                                <a href="{{route('admin.expertises.edit',['id' => $expertis->id])}}"
                                   class="btn btn-xs btn-success"><i class="fad fa-edit mr-1"></i> Düzenle</a>
                                <a href="javascript:;" onclick="deleteCustomer({{$expertis->id}});"
